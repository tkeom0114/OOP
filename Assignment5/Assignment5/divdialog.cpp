...
DivDialog::DivDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::DivDialog)
{
    ui->setupUi(this);
...
void DivDialog::changeLabel1(QString name)
{
    QString curstring;
    list<Operand*>::iterator iterEnd=Operands.end();
...
void DivDialog::changeLabel2(QString name)
{
    QString curstring;
    list<Operand*>::iterator iterEnd=Operands.end();
    list<Operand*>::iterator iterPos;
...
void DivDialog::on_pushOK_2_clicked()
{
    list<Operand*>::iterator iterEnd=Operands.end();
    ...
    if((*iterPos2)->getType()=="Numeric" && dynamic_cast<Numeric*>(*iterPos2)->getValue()<0.0000000001
            && dynamic_cast<Numeric*>(*iterPos2)->getValue()>-0.0000000001)
    {
        QMessageBox::warning(this,"Error","Cannot Calculate");
        return;
    }
    ...
    {
        if(dynamic_cast<Numeric*>(*iterPos1)->getValue()<0.0000000001
                && dynamic_cast<Numeric*>(*iterPos1)->getValue()>-0.0000000001)
        {
            QMessageBox::warning(this,"Error","Cannot Calculate");
            return;
        }
